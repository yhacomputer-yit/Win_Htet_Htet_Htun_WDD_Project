...
$get_bag_cat = mysqli_query($conn, "SELECT CAT_ID FROM CATEGORYLIST WHERE CAT_NAME LIKE '%Bag%' LIMIT 1");
$bag_data = mysqli_fetch_assoc($get_bag_cat);
$bag_id = $bag_data['CAT_ID'] ?? 0;
...
if (isset($_POST['save_product'])) {
    ...
    if (move_uploaded_file($tmp_name, $upload_path)) {
        ...
        if (mysqli_query($conn, $sql_insert)) {
            echo "<script>alert('Bag product saved successfully!'); window.location='productbag.php';</script>";
        } else {
    ...
    <div class="row">
       <?php include('sidebar.php'); ?>
        ...
        <div class="col-lg-10 p-4">
            <h3 class="mb-4 text-secondary">Bag Management</h3>

            <div class="card shadow-sm border-0 mb-5 p-4">
                <h5 class="mb-3 text-danger"><i class="fa fa-plus-circle"></i> Add New Bag</h5>
                <form action="" method="POST" enctype="multipart/form-data">
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            <label class="form-label fw-bold">Category (Bag Only)</label>
                            <select name="cat_id" id="bagCategory" class="form-select" required>
                                <option value="">Loading Bag Category...</option>
                                <?php
                        ...
                        <div class="col-md-3 d-flex align-items-end px-3">
                            <div class="form-check mb-2">
                                ...
                                <label class="form-check-label fw-bold text-danger" for="specialCheck">Special?</label>
                            </div>
                        </div>
                        <div class="col-12 text-end mt-3">
                            <button type="submit" name="save_product" class="btn btn-danger px-4">Save Bag</button>
                        </div>
                    </div>
                </form>
            </div>
            ...
            <h4 class="text-secondary mb-3">Bag List</h4>
            <div class="table-responsive bg-white p-3 rounded shadow-sm">
                <table class="table table-hover align-middle">
                    ...
                    <tbody>
                        <?php
                        $sql_select = "SELECT * FROM PRODUCTLIST WHERE CAT_ID = '$bag_id' ORDER BY PRODUCT_ID DESC";
                        ...
                        if(mysqli_num_rows($result) > 0) {
                            ...
                            while ($row = mysqli_fetch_assoc($result)) {
                                echo "<tr>
                                    <td>{$i}</td>
                                    <td><img src='uploads/{$row['PRO_IMG']}' width='50' height='50' class='rounded border'></td>
                                    <td class='fw-bold'>{$row['PRO_NAME']}</td>
                                    <td>".number_format($row['PRO_PRICE'])." MMK</td>
                                    <td>".(($row['IS_SPECIAL'] == 1) ? '<span class="badge bg-warning text-dark">Special</span>' : '<span class="badge bg-light text-muted">Normal</span>')."</td>
                                    <td>
                                        <a href='products_edit.php?id={$row['PRODUCT_ID']}' class='btn btn-sm btn-outline-success'><i class='fa fa-edit'></i></a>
                                        <a href='product_delete.php?id={$row['PRODUCT_ID']}' class='btn btn-sm btn-outline-danger' onclick='return confirm(\"ဖျက်ရန် သေချာပါသလား?\")'><i class='fa fa-trash'></i></a>
                                    </td>
                                </tr>";
                                $i++;
                            }
                        } else {
                            echo "<tr><td colspan='6' class='text-center py-4'>No bags found.</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
...
document.addEventListener("DOMContentLoaded", function() {
    const select = document.getElementById('bagCategory');
    const options = select.options;

    for (let i = options.length - 1; i >= 0; i--) {
        const catName = options[i].getAttribute('data-name');
        
        if (catName && catName.includes('bag')) {
