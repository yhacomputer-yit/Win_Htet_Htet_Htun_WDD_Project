...
$cat_res = mysqli_query($conn, "SELECT CAT_ID FROM CATEGORYLIST WHERE CAT_NAME LIKE '%Doll%' LIMIT 1");
$cat_data = mysqli_fetch_assoc($cat_res);
$doll_id = $cat_data['CAT_ID'] ?? 0;
...
if(isset($_POST['add_to_cart'])) {
    ...
    if(!in_array($product_id, $ids)) {
        ...
        echo "<script>alert('Added to cart!'); window.location.href='productsdoll.php';</script>";
    } else {
...
<body style="background-color: #fde2e4;">
        ...
        <div class="bg-danger text-white p-5 rounded-5 shadow-sm">
            <div class="row align-items-center">
                <div class="col-md-7 text-center text-md-start">
                    <h2 class="fw-bold display-6 mb-3 border-bottom border-white d-inline-block pb-2">Handmade Dolls</h2>
                    <p class="fs-5 fw-medium mt-2">Soft, huggable, and thoughtfully handcrafted — perfect companions for everyone.</p>
                </div>
    ...
    <div class="container py-5">
        <h3 class="text-secondary fw-bold mb-4"><i class="fa-solid fa-face-smile-beam"></i> Doll Collection</h3>
        <div class="row g-4">
            <?php
            // Database ထဲက Category က Doll ဖြစ်တဲ့ ပစ္စည်းတွေကိုပဲ ဆွဲထုတ်ပြမယ်
            $res = mysqli_query($conn, "SELECT * FROM PRODUCTLIST WHERE CAT_ID = '$doll_id'");
                ...
                <form method="POST">
                    <div class="bg-white p-3 rounded-4 shadow-sm h-100 d-flex flex-column product-card">
                        <div class="ratio ratio-1x1 mb-3 overflow-hidden rounded-3 shadow-sm">
                            <img src="uploads/<?= trim($row['PRO_IMG']); ?>" onerror="this.src='https://via.placeholder.com/300x300?text=Doll+Item'" class="w-100" style="object-fit: cover;">
                        </div>
            ...
            <?php 
                ...
                echo "<div class='col-12 text-center'><p class='text-muted'>No dolls found in this category.</p></div>";
            }
            ?>
        </div>
    </div>
    ...
    <div class="container-fluid py-5 bg-danger-subtle rounded-top-5">
        <div class="container text-center">
            <h2 class="fw-bold mb-5 border-bottom border-dark d-inline-block pb-2">Special Dolls For You</h2>
            <div class="row justify-content-center g-4">
                <?php
                // Special check လုပ်ထားတဲ့ Doll တွေကိုပဲ ထုတ်ပြမယ်
                $special_res = mysqli_query($conn, "SELECT * FROM PRODUCTLIST WHERE CAT_ID = '$doll_id' AND IS_SPECIAL = 1 LIMIT 3");
