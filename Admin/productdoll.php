...
$get_doll_cat = mysqli_query($conn, "SELECT CAT_ID FROM CATEGORYLIST WHERE CAT_NAME LIKE '%Doll%' LIMIT 1");
$doll_data = mysqli_fetch_assoc($get_doll_cat);
$doll_id = $doll_data['CAT_ID'] ?? 0;
...
if (isset($_POST['save_product'])) {
    ...
    if (move_uploaded_file($tmp_name, $upload_path)) {
        ...
        if (mysqli_query($conn, $sql_insert)) {
            echo "<script>alert('Doll saved successfully!'); window.location='productdoll.php';</script>";
        } else {
    ...
    <div class="row">
      <?php include('sidebar.php'); ?>
        ...
        <div class="col-lg-10 p-4">
            <h3 class="mb-4 text-secondary">Doll Management</h3>

            <div class="card shadow-sm border-0 mb-5 p-4">
                <h5 class="mb-3 text-danger"><i class="fa fa-plus-circle"></i> Add New Doll</h5>
                <form action="" method="POST" enctype="multipart/form-data">
                    <div class="row g-3">
                        <div class="col-md-4">
                            <label class="form-label fw-bold">Doll Name</label>
                            <input type="text" name="pro_name" class="form-control" required>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label fw-bold">Category (Doll Only)</label>
                            <select name="cat_id" id="dollCategoryFilter" class="form-select" required>
                                <?php
                        ...
                        <div class="col-12 text-end mt-3">
                            <button type="submit" name="save_product" class="btn btn-danger px-4">Save Doll</button>
                        </div>
                    </div>
                </form>
            </div>
            ...
            <h4 class="text-secondary mb-3">Doll List</h4>
            <div class="table-responsive bg-white p-3 rounded shadow-sm">
                <table class="table table-hover align-middle">
                    ...
                    <tbody>
                        <?php
                        $sql_select = "SELECT * FROM PRODUCTLIST WHERE CAT_ID = '$doll_id' ORDER BY PRODUCT_ID DESC";
                        ...
                        if(mysqli_num_rows($result) > 0) {
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php echo ($row['IS_SPECIAL'] == 1) ? '<span class="badge bg-warning text-dark">Special</span>' : '<span class="badge bg-light text-muted">Normal</span>'; ?>
                                </td>
                                <td>
                            ...
                            echo "<tr><td colspan='6' class='text-center py-4'>No dolls found.</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
...
document.addEventListener("DOMContentLoaded", function() {
    const select = document.getElementById('dollCategoryFilter');
    const options = select.options;
    for (let i = options.length - 1; i >= 0; i--) {
        const catName = options[i].getAttribute('data-name');
        
        if (catName && catName.includes('doll')) {
